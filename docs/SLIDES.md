# Slides Ejecutivos, Caso Ferreycorp (máximo 7 slides)
...
**Objetivos técnicos**
1. AUC-ROC mayor o igual a 0.65, Lift @ top-10% mayor o igual a 2.0
2. Modelo explicable (SHAP) global y por cliente
3. Agente conversacional sobre predicciones (acceso menor a 100 ms)
...
| Semana | Entregable |
|---|---|
| **S1** | EDA y diseño de features causales |
| **S2** | Modelo (baseline + LightGBM + tuning) |
| **S3** | Explicabilidad y agente IA |
| **S4** | UI Streamlit, despliegue cloud y documentación |
...
|---|---|
| **89%** de clientes concentran más del 50% de compras en una marca | Lealtad histórica como feature top |
...
| Cliente promedio: 117 visitas y 29 compras en 2 años | Densidad temporal suficiente para features causales |
...
- **Sensibilidad a promo**: tasa de compra con o sin promo, uplift personal
- **Precio relativo**: precio de hoy vs. promedio histórico por marca
- **Demografía y cluster**: K-Means k=4 produce 4 segmentos de cliente

**Modelos**: Logistic Regression (baseline) y luego **LightGBM + Optuna (30 trials)**

**Validación**: split temporal, métrica objetivo: AUC-ROC y Lift @ top-K
...
|---|---|---|---|---|
| **0.684** | 0.457 | 0.170 | **2.40 veces** | 1.90 veces |
...
Llamando solo al top 20%, se capturan aprox. **38% de las compras totales**.
...
**Cloud-agnóstico**: el mismo Docker corre en DO, AWS, GCP, solo cambia el `.env`.
...
**Seguridad**: el agente no escribe SQL libre, emite filtros estructurados que se validan contra whitelist y se traducen a SQL parametrizado.
...
|---|---|---|
| 📣 Marketing dirigido | Cortar contactos a bottom-50% | Costo por compra: menos 50% |
| 🏷️ Pricing y promo | Personalizar promos por segmento | Lift incremental por promo |
| ♻️ Reactivación | Detectar baja-propensión activa | Tasa de reactivación: más 10pp |
| ⚡ Operación comercial | Self-service vía chat | Time-to-insight: de horas a segundos |
...
- Rentabilidad por contacto aprox. **5 veces** vs. baseline

**Roadmap próximas fases**: modelo de marca, A/B testing, monitoreo de drift, pricing optimizer
